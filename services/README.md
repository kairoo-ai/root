...
| `services/entitlements`  | What a user is _allowed_ to do (feature gates derived from plan).     |
...
1. **`services/usage`** - the _source of truth_. Tracks per-user spend/quota and
   answers "how much budget is left?" (`getRemainingBudget`, `recordUsage`).
2. **`services/ai`** - the _enforcement point_. Before running a feature,
