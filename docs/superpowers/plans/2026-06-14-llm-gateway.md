...
export function compose(opts: {
  ...
  const system = [BASE_PROMPT, opts.systemAddendum]
    .filter(Boolean)
    .join("\n\n");
    ...
    for (let attempt = 0; attempt <= RETRIES; attempt++) {
      ...
      try {
        const res = await withTimeout(
          adapter.generate(req, cand.model),
          TIMEOUT_MS,
        );
...
export async function POST(request: NextRequest) {
  ...
  if (!tool)
    return NextResponse.json(
      { error: `Unknown tool: ${parsed.value.toolId}` },
      { status: 400 },
    );
  ...
  const messages = compose({
    systemAddendum: tool.systemAddendum,
    userPrompt,
    context,
  });
  ...
  return NextResponse.json({
    result: sanitizeOutput(result.value.text) || "AI response unavailable.",
  });
}
```
