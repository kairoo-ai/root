...
export function computeProgress(plan: RoadmapPlanJson): number {
  ...
  return Math.round(
    (all.filter((s) => s.status === "done").length / all.length) * 100,
  );
}
```
...
export async function PATCH(
  ...
  if (!body?.stepId || !body?.phaseId || !body?.status) {
    return NextResponse.json(
      { error: "Missing stepId, phaseId, or status" },
      { status: 400 },
    );
  }
  ...
  const updated = await patchStepStatus(
    id,
    userId,
    body.phaseId,
    body.stepId,
...
function isRawPhasedJson(x: unknown): x is RawPlan {
  return (
    ...
    x !== null &&
    "phases" in x &&
    Array.isArray((x as RawPlan).phases)
  );
}
...
function normalizeStep(s: RawStep): RoadmapStep {
  return {
    ...
    resources: (s.resources ?? []).map((r) => ({
      title: r.title ?? "",
      url: r.url ?? "",
      type: (["article", "video", "course", "book"].includes(r.type ?? "")
        ? r.type
        : "article") as RoadmapStep["resources"][number]["type"],
    })),
  ...
  async function generate() {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res
          .json()
          .catch(() => ({ error: "Failed to generate roadmap" }));
        ...
        >
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Be specific - this drives the entire roadmap.
              </p>
              <input
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                This sets the pace and depth of your roadmap.
              </p>
...
export async function POST(req: NextRequest) {
  ...
  if (!title || !goal || !planJson) {
    return NextResponse.json(
      { error: "Missing title, goal, or planJson" },
      { status: 400 },
    );
  }
...
function StepStatusIcon({ status }: { status: RoadmapStep["status"] }) {
  if (status === "done")
    return <CheckCircle2 className="w-4 h-4 text-teal-400 shrink-0" />;
...
interface FlatStep extends RoadmapStep {
  phaseId: string;
  phaseTitle: string;
}
...
export function RoadmapKanban({ phases, onStatusChange, onStepClick }: Props) {
  ...
  const byStatus = (status: StepStatus) =>
    flat.filter((s) => s.status === status);
      ...
      <div className="flex gap-6 min-w-max">
        {phases.map((phase, pi) => (
          <div key={phase.id} className="flex flex-col gap-1">
            ...
            <div
              className={cn(
                "text-[10px] font-bold px-3 py-1 rounded-lg border mb-2 self-start",
                PHASE_BORDER[phase.color] ?? PHASE_BORDER.teal,
              )}
            >
              {phase.title}
            </div>
...
const RESOURCE_ICONS: Record<
  RoadmapStep["resources"][number]["type"],
  React.ElementType
> = {
  article: FileText,
  video: Video,
            ...
            <div className="flex items-start justify-between p-5 border-b border-border">
              <div>
                ...
                <div className="text-[10px] text-teal-400 mt-1 font-medium">
                  {step.duration} · +{step.xpReward} XP
                </div>
              </div>
              <button
                onClick={onClose}
                    ...
                    {step.resources.map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-xs text-foreground/80 flex-1 line-clamp-1">
                            {r.title}
                          </span>
...
export function RoadmapDetail({ roadmap }: { roadmap: Roadmap }) {
  ...
  const handleStatusChange = useCallback(
    async (phaseId: string, stepId: string, status: RoadmapStep["status"]) => {
      // Optimistic update
      setPlan((prev) => {
        if (!prev) return prev;
        return {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {plan ? (
        ...
        >
          {view === "timeline" && (
            <RoadmapTimeline phases={plan.phases} onStepClick={openStep} />
          )}
          {view === "kanban" && (
            <RoadmapKanban
              phases={plan.phases}
          ...
          {legacyPlan.steps.map((step, i) => (
            ...
            >
              <div className="flex flex-col items-center gap-1">
                <div className="w-7 h-7 rounded-full bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-xs font-bold text-teal-400 shrink-0">
                  {i + 1}
                </div>
                {i < (legacyPlan.steps?.length ?? 0) - 1 && (
              ...
              <div className="flex-1 pb-2">
                ...
                {step.duration && (
                  <div className="text-[10px] text-teal-400 mt-2 font-medium">
                    {step.duration}
                  </div>
                )}
              </div>
