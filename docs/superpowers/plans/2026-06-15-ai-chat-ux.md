...
function extractFollowUps(content: string, featureName: string): string[] {
  ...
  if (lines.length >= 2) {
    return lines.slice(-3).map(l => l.replace(/^[\d\-\*\.]+\s*/, '').trim()).slice(0, 3)
  }
  // Generic fallback
  return [
  ...
  const sendMessage = useCallback(async (content: string, isFirstMessage = false) => {
    ...
    try {
      ...
      setDisplayMessages(prev => {
        ...
        return [...prev.filter(m => m.id !== optimisticUserMsg.id), optimisticUserMsg, assistantMsg]
      })
      ...
      setThreads(prev =>
        prev.map(t =>
          t.id === threadId
            ? { ...t, messages: [...t.messages, optimisticUserMsg, assistantMsg], updatedAt: new Date().toISOString() as unknown as Date }
            : t
        )
      )
