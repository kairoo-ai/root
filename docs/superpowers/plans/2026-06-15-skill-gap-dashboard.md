...
async function computeGapsViaAI(
  ...
  baseUrl: string,
): Promise<{ targetSkills: TargetSkillEntry[]; gaps: import('@/data/schema').SkillGap[] }> {
...
Include 8-12 target skills. Only include gaps where delta > 0. Sort gaps by priority then delta desc.`
  ...
  return JSON.parse(cleaned) as { targetSkills: TargetSkillEntry[]; gaps: import('@/data/schema').SkillGap[] }
}
